...
+(instancetype)shared
{
	...
	dispatch_once(&onceToken, ^{
        instance = [[[self alloc] init] initWithDatabaseFilename:dbName];
	});
...
-(void)executeExecutableQuery{
    [self runQuery:[preparedQuery UTF8String] isQueryExecutable:YES];
    preparedQuery = @"";
}

-(NSArray *)executeNonExecutableQuery{
    [self runQuery:[preparedQuery UTF8String] isQueryExecutable:NO];
    preparedQuery = @"";
    return (NSArray *)self.arrResults;
}
...
-(void)updateQuery:(NSString*)table table:(NSMutableArray*)setParameters setParameters:(NSMutableArray*)conditionParameters{
    preparedQuery = [NSString stringWithFormat:@"UPDATE %@ SET ", table];
    [self createCommaParameters:setParameters];
    [self createConditionParameters:conditionParameters];
}
...
-(void)createConditionParameters:(NSMutableArray*)conditionParameters{
    ...
    {
        preparedQuery = [NSString stringWithFormat:@"%@ WHERE", preparedQuery];
        
        for (NSString *curr in conditionParameters)
        {
            preparedQuery = [NSString stringWithFormat:@"%@ %@", preparedQuery, curr];
            NSString *and = [conditionParameters indexOfObject:curr] == conditionParameters.count - 1 ? @"" : @" AND ";
            preparedQuery = [NSString stringWithFormat:@"%@ %@", preparedQuery, and];
        }
    }
}
...
-(void)orderBy:(NSMutableArray*)values values:(NSString*)method{
    preparedQuery = [NSString stringWithFormat:@"%@ ORDER BY ", preparedQuery];
    [self createCommaParameters:values];
    preparedQuery = [NSString stringWithFormat:@"%@ %@", preparedQuery, method];
}
